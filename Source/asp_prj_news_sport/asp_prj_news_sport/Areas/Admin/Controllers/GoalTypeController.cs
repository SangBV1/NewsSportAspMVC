...
namespace asp_prj_news_sport.Areas.Admin.Controllers
{
    public class GoalTypeController : BaseController
    {
        ...
        {
            var model = new goal_type_dao().Get_By_Id(id);
            return View(model);
        }

        [HttpPost]
        [HasCredential(RoleID = CommonPublic.CommonConstants.ADD_MENU_TYPE)]
        public ActionResult Create(goal_type type)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", StaticResources.Resources.InsertGoalTypeFailed);
                }
            }
            return View();
        }
        ...
        [HttpPost]
        [HasCredential(RoleID = CommonPublic.CommonConstants.EDIT_MENU_TYPE)]
        public ActionResult Edit(goal_type type)
        {
            if (ModelState.IsValid)
            {
                var dao = new goal_type_dao();
                ...
                {
                    ModelState.AddModelError("", StaticResources.Resources.UpdateGoalTypeFailed);
                }
            }
            return View();
        }
